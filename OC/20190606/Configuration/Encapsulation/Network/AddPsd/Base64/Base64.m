...
- (NSString *)authCode:(NSString *)auth_key operation:(NSStringAuthCode)operation encoding:(NSStringEncoding)encoding
{
    ...
    NSRange rangeB = {16,16};
    //keyB
    NSString *keyB = [[key substringWithRange:rangeB]md5];
    //keyC
    NSString *keyC = nil;
    //随时间变化的随机字符串
    NSString   *time_stamp;
    ...
    time(&now);
    //时间戳
    time_stamp  = [[NSString stringWithFormat:@"%ld", now]md5];
    NSRange rangeC = {time_stamp.length-4,4};
    keyC = [time_stamp substringWithRange:rangeC];
    
    NSString *criptKey = [NSString stringWithFormat:@"%@%@",keyA,[[NSString stringWithFormat:@"%@%@",keyA,keyC]md5]];
    NSInteger criptKey_length = criptKey.length;
...
//    NSString *string = (operation == NSStringAuthCodeDecoded) ? [self base64DecodedString:encoding] : [NSString stringWithFormat:@"%@%@",[[[NSString stringWithFormat:@"%@%@",self,key] md5] substringToIndex:8],self];
    //（明文+秘钥MD5加密后的字符串，再进行MD5加密获取前八位）+明文
    NSString *string_one = [[[NSString stringWithFormat:@"%@%@",self,keyB]md5] substringToIndex:16];
    NSString *string = [NSString stringWithFormat:@"%@%@%@",[NSString stringWithFormat:@"0000000000"],string_one,self];
    
    NSUInteger string_length = string.length;
    for(int i = 0; i <= 255; i++) {
        [rndkey addObject:[NSNumber numberWithUnsignedShort:[criptKey characterAtIndex:i%criptKey_length]]];
    ...
    if(operation == NSStringAuthCodeDecoded) {
        ...
        NSString *outPutString = [NSString stringWithFormat:@"%@%@",keyC,[[result base64EncodedString:encoding] stringByReplacingOccurrencesOfString:@"=" withString:@""]];
        
        outPutString = [outPutString stringByReplacingOccurrencesOfString:@"+" withString:@"_"];
        return outPutString;
    }
}
