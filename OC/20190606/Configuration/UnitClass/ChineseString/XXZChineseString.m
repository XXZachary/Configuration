...
#import "XXZChineseString.h"
#import "Pinyin.h"

@implementation XXZChineseString
...
+ (NSArray *)sortArray:(NSArray *)stringArr {
    ...
    for(int i=0; i<[stringArr count]; i++){
        XXZChineseString *chineseStr = ((XXZChineseString*)[tempArray objectAtIndex:i]);
        [result addObject:chineseStr.string];//取文本
        
//        if (IS_BLANK_STRING(chineseStr.pinYin)) {
//            continue;
//        }
...
+ (NSMutableArray *)letterSortArray:(NSArray *)stringArr {
    ...
    for (NSObject *object in tempArray) {
        NSString *pinyin = [((XXZChineseString*)object).pinYin substringToIndex:1];
        NSString *contentStr = ((XXZChineseString*)object).string;
        //不同
...
+ (NSMutableArray *)returnSortChineseArray:(NSArray *)stringArr {
    ...
    for(int i=0;i<[stringArr count];i++) {
        XXZChineseString *chineseString=[[XXZChineseString alloc]init];
