...
#pragma mark - Global Color
    ...
    QMUICMI.backgroundColor = UIColorMake(247, 247, 247);                       // UIColorForBackground : 界面背景色，默认用于 QMUICommonViewController.view 的背景色
...
#pragma mark - NavigationBar
    ...
    //导航栏
//    UIImage *image = [UIImage imageNamed:@"navigationbar_background_new"];
//    NSInteger leftCapWidth = image.size.width * 0.5; // 设置左边端盖宽度
//    NSInteger topCapHeight = image.size.height * 0.5; // 设置上边端盖高度
//    UIImage *newImage = [image stretchableImageWithLeftCapWidth:leftCapWidth topCapHeight:topCapHeight];
    
    QMUICMI.navBarBackgroundImage = [self navigationImage]; //[[UIImage imageNamed:@"navigationbar_background_new"] imageByResizeToSize:CGSizeMake(SCREEN_WIDTH, 76)]; // NavBarBackgroundImage : UINavigationBar 的背景图
    
    //导航栏阴影
    UIImage *shadow_image = [UIImage imageNamed:@"navigationbar_shadow_new"];
    NSInteger shadow_leftCapWidth = shadow_image.size.width * 0.5; // 设置左边端盖宽度
    NSInteger shadow_topCapHeight = shadow_image.size.height * 0.5; // 设置上边端盖高度
    UIImage *shadow_newImage = [shadow_image stretchableImageWithLeftCapWidth:shadow_leftCapWidth topCapHeight:shadow_topCapHeight];
    
    QMUICMI.navBarShadowImage = shadow_newImage;                                  // NavBarShadowImage : UINavigationBar.shadowImage，也即导航栏底部那条分隔线
...
#pragma mark - TabBar
    ...
    QMUICMI.tabBarTintColor = UICOLOR_FROM(@"#979596");                              // TabBarTintColor : UITabBar 的 tintColor
    QMUICMI.tabBarItemTitleColor = UICOLOR_FROM(@"#FF2365");                                // TabBarItemTitleColor : 未选中的 UITabBarItem 的标题颜色
...
- (UIColor *)themeTintColor {
    return [UIColor colorWithRed:12/255.0f green:111/255.0f blue:197/255.0f alpha:1.0f];
}
...
- (UIImage *)navigationImage {
    CGRect rect = CGRectMake(0, 0, SCREEN_WIDTH, Safe_Navbar_H);
    UIGraphicsBeginImageContext(rect.size);
    
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextSetFillColorWithColor(context, THEME_COLOR.CGColor);
